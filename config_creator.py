...
class ConfigCreator:
    ...
    def create_config_object(config):
        config_object = {"input":[],"output":"som","name":None}
        ...
        for a_prop in ["output","train_scenes","test_scene","name"]:
            if a_prop in config:
        ...
        if config_object["name"] is None:
            ...
            scene_part = config_object['train_scenes']
            if type(scene_part) == list:
                scene_part = len(config_object['train_scenes'])
